...
`;
                                ...
                                fetch('https://api.github.com/repos/Mischlichter/data/contents/gallerycom')
                                    ...
                                    .then(files => {
                                        ...
                                        function loadImage(index) {
                                            ...
                                            img.setAttribute('data-fullsize', file.download_url); // Store the full-size image URL
                                            ...
                                            dbRequest.onsuccess = function(event) {
                                                ...
                                                if (dateStr) {
                                                    ...
                                                    lastModifiedCurrent = new Date(0);
                                                }
                                                ...
                                                if (!dbResult || lastModifiedInDB < lastModifiedCurrent) {
                                                    fetch(file.download_url)
                                                        ...
                                                        .then(blob => {
                                                            ...
                                                            createThumbnail(dataUrl, img); // Create and set the thumbnail
                                                            dynamicImages.push(file.download_url); // Store the full-size image URL
                                                        })
                                                ...
                                                img.onload = () => {
                                                    ...
                                                    img.onclick = () => onImageClick(file.download_url); // Use full-size image for slideshow
        ...
        // Create a thumbnail and set it as the source of the img element
        function createThumbnail(imageSrc, imgElement) {
            const canvas = document.createElement('canvas');
            const ctx = canvas.getContext('2d');
            const thumbnailWidth = 150; // Set the desired width for the thumbnail
            const thumbnailHeight = 150; // Set the desired height for the thumbnail
            ...
            img.onload = function () {
                ...
                imgElement.src = canvas.toDataURL(); // Set the thumbnail as the source of the img element
            };
        }
       ...
       function updateVisibleImages() {
           const allImages = document.querySelectorAll('#gallery-container .image-container img');
           ...
           allImages.forEach(img => {
               ...
               if (isVisible) {
                   newVisibleImages.push(img.getAttribute('data-fullsize')); // Use full-size URL for visible images
               }
           });
        ...
        function updateTextInfo() {
            ...
            const imageElement = document.querySelector(`img[data-fullsize="${imgSrc}"]`);
        ...
        function updateTextInfo2() {
            const imgSrc = dynamicImages[currentImageIndex];
            const imageElement = document.querySelector(`img[data-fullsize="${imgSrc}"]`); // Find the img element by its data-fullsize attribute
