...
let serviceWorkerEnabled = true; // Default to enabled
...
self.addEventListener('fetch', event => {
    if (!serviceWorkerEnabled) {
        console.log('Service Worker: Disabled, bypassing fetch for', event.request.url);
        return;
    }
...
self.addEventListener('message', event => {
    if (event.data.action === 'setState') {
        serviceWorkerEnabled = event.data.enabled;
        console.log('Service Worker: State set to', serviceWorkerEnabled);
    }
});
